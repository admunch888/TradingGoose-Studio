 ...
 *
 * The Kronos Signal route: auth, body validation, and the flat signal it returns.
 *
 ...
 * bad threshold is a bad threshold) and what it answers instead of refusing (a series
 * too short to estimate volatility from, a forecast with no ensemble: a flat signal
 * with the reason, which is what an operator wired into a workflow needs to read).
 */
