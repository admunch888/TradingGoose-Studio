...
- The response then also carries a top-level `ensemble: { sampleCount, shareUp }`: `shareUp` is the share of sampled paths whose **terminal** close is above the last historical close, counted strictly (`>`), so a "75% of samples must agree on direction" rule can be evaluated from it - a p10/p90 band can only ever answer "at least 90%". Like the band, the key is **omitted when `sampleCount = 1`**, and `ensemble.sampleCount` is the count actually reduced, which today matches `parameters.sampleCount`.
